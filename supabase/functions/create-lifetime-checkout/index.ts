...
// Price ID for Pro Plus Lifetime ($59 one-time)
...
serve(async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${origin}/dashboard?lifetime=success&session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${origin}/lifetime?canceled=true`,
      metadata: {
        user_id: user.id,
